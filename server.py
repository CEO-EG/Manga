...
.nav{display:flex;flex-direction:column;gap:4px}.nav-label{padding:10px 10px 6px;color:var(--dim);font:800 10px/1 var(--mono);letter-spacing:.14em;text-transform:uppercase}.nav-item{position:relative;width:100%;display:flex;align-items:center;gap:11px;padding:11px 12px;border:1px solid transparent;border-radius:11px;background:transparent;color:var(--muted);font-size:14px;font-weight:700;text-align:left;cursor:pointer;transition:.16s ease}.nav-item:hover{background:var(--overlay);color:var(--text);border-color:rgba(255,255,255,.04)}.nav-item.active{background:rgba(26,30,38,.78);color:var(--text);border-color:rgba(255,255,255,.06);box-shadow:inset 3px 0 0 rgba(244,244,245,.82),0 0 22px rgba(255,255,255,.025)}.nav-icon,.bn-ico{display:inline-grid;place-items:center;flex:0 0 auto}.nav-icon svg{width:19px;height:19px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}.nav-badge{margin-left:auto;display:none;min-width:20px;height:20px;border-radius:999px;background:#3f3f46;color:#fff;font:800 10px/20px var(--mono);text-align:center}
...
.bot-nav{display:none;position:fixed;left:0;right:0;bottom:0;z-index:200;padding:8px 10px calc(8px + env(safe-area-inset-bottom));background:linear-gradient(to top,rgba(7,7,8,.98),rgba(7,7,8,.82),transparent)}.bot-nav-inner{height:58px;display:grid;grid-template-columns:repeat(4,1fr);gap:5px;padding:5px;border:1px solid var(--line-soft);border-radius:15px;background:#111114;box-shadow:0 10px 40px rgba(0,0,0,.42)}.bn{position:relative;border:0;background:transparent;color:var(--dim);border-radius:11px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;font-size:10px;font-weight:900;cursor:pointer}.bn:hover{background:rgba(26,30,38,.62);color:#d4d4d8}.bn.active{background:rgba(26,30,38,.86);color:#f4f4f5;box-shadow:inset 0 2px 0 rgba(244,244,245,.78)}.bn svg{width:20px;height:20px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}.bn-bdg{position:absolute;top:4px;right:20%;display:none;min-width:18px;height:18px;border-radius:999px;background:#3f3f46;color:#fff;font:900 9px/18px var(--mono)}#toast{position:fixed;right:16px;bottom:16px;z-index:600;max-width:320px;opacity:0;transform:translateY(8px);pointer-events:none;transition:.18s ease;padding:12px 14px;border:1px solid var(--line);border-radius:12px;background:#151519;box-shadow:var(--shadow);font:800 12px/1.4 var(--mono);color:#f4f4f5}#toast.show{opacity:1;transform:none}.empty{grid-column:1/-1;padding:52px 16px;border:1px dashed var(--line);border-radius:16px;background:#101013;color:var(--dim);font:800 13px/1.5 var(--mono);text-align:center}
  ...
  grid.innerHTML=d.manga.map(m=>{
    ...
    return`<article class="m-card">
      ...
      <div class="m-actions">
        ...
        <div class="m-del-row">
          <button class="btn btn-del" type="button" title="Delete original manga folder" aria-label="Delete original manga folder" data-action="delete" data-name="${nameAttr}" data-label="${nameAttr} — original manga folder">${ICONS.trash}<span class="del-label">Delete original</span><span class="del-small">source files</span></button>
          ${m.has_opt?`<button class="btn btn-del" type="button" title="Delete optimized WebP folder" aria-label="Delete optimized WebP folder" data-action="delete" data-name="${escAttr(m.name+'-optimized')}" data-label="${nameAttr} — optimized WebP folder">${ICONS.trash}<span class="del-label">Delete optimized</span><span class="del-small">WebP folder</span></button>`:''}
          ${m.has_cbz?`<button class="btn btn-del" type="button" title="Delete CBZ export folder" aria-label="Delete CBZ export folder" data-action="delete" data-name="${escAttr(m.name+'-cbz')}" data-label="${nameAttr} — CBZ export folder">${ICONS.trash}<span class="del-label">Delete CBZ export</span><span class="del-small">packed files</span></button>`:''}
        </div>
      </div>
    </article>`;
  }).join('');
